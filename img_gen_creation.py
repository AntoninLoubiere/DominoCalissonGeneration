    ...
    for j in range(3 * SIZE // 2  if CROPPED else SIZE):
        ...
        for i in range(SIZE // 2, 3 * SIZE // 2) if CROPPED else range(SIZE):
            ...
            if l[i] == ' ':
                ...
                c = COLORS[0] if (i + j + SIZE - size) % 2 == 1 else COLORS[1]
            elif l[i] == 'v':
                c = COLORS[0] if (i + j + SIZE - size) % 2 == 0 else COLORS[1]
            elif l[i] == '<':
                c = COLORS[2] if (i + j + SIZE - size) % 2 == 0 else COLORS[3]
            elif l[i] == '>':
                c = COLORS[2] if (i + j + SIZE - size) % 2 == 1 else COLORS[3]
            else:
                c = "#111"
